...
class StackingRegressorSuite extends FunSuite with DatasetSuiteBase {
  ...
  test("benchmark") {
    ...
    val sr = new StackingRegressor().setStacker(new DecisionTreeRegressor()).setLearners(Array(new DecisionTreeRegressor(),new RandomForestRegressor())).setFeaturesCol("features").setLabelCol("medv").setParallelism(4)
